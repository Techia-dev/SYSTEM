...
class CandidatesBloc extends Bloc<CandidatesEvent, CandidatesState> {
  ...
  CandidatesBloc({CandidatesRepository? repository})
      : _repository = repository ?? sl<CandidatesRepository>(),
  ...
  void _onUpdateFilter(CandidatesUpdateFilter event, Emitter<CandidatesState> emit) {
    final f = ListFilter(
      searchQuery: event.searchQuery ?? state.filter.searchQuery,
      status: event.status ?? state.filter.status,
      level: event.level,
      page: event.page ?? 1,
  ...
  Future<void> _onDelete(CandidatesDelete event, Emitter<CandidatesState> emit) async {
    try {
      ...
      if (msg.contains('violates RESTRICT') || msg.contains('foreign key constraint')) {
        emit(state.copyWith(
          error: 'Cannot delete candidate: they have existing commissions. Remove those commissions first.',
        ));
      } else {
